...
def register_financial_report_tools(app: FastMCP, active_data_source: FinancialDataSource):
    ...
    @contract_tool(app)
    def get_profit_data(code: StockCode, year: Year, quarter: Quarter) -> str:
        """
    ...
    @contract_tool(app)
    def get_operation_data(code: StockCode, year: Year, quarter: Quarter) -> str:
        """
        获取股票的季度营运能力数据（如周转率等）
    ...
    @contract_tool(app)
    def get_growth_data(code: StockCode, year: Year, quarter: Quarter) -> str:
        """
    ...
    @contract_tool(app)
    def get_balance_data(code: StockCode, year: Year, quarter: Quarter) -> str:
        """
    ...
    @contract_tool(app)
    def get_cash_flow_data(code: StockCode, year: Year, quarter: Quarter) -> str:
        """
    ...
    @contract_tool(app)
    def get_dupont_data(code: StockCode, year: Year, quarter: Quarter) -> str:
        """
        获取股票的季度杜邦分析数据（ROE分解）
