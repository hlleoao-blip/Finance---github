...
from src.tools.contracts import OptionalISODate, StockCode, ToolExecutionError, contract_tool
...
def register_index_tools(app: FastMCP, active_data_source: FinancialDataSource):
    ...
    def get_stock_industry(
        ...
        try:
            ...
            raise ToolExecutionError(
                "INTERNAL_ERROR",
                "Unexpected failure while executing get_stock_industry.",
                details={"exception_type": type(e).__name__},
            ) from e

    @contract_tool(app)
    def get_sz50_stocks(date: OptionalISODate = None) -> str:
        """
        获取指定日期的深证50指数成分股数据
    ...
    @contract_tool(app)
    def get_hs300_stocks(date: OptionalISODate = None) -> str:
        """
        获取指定日期的沪深300指数成分股数据
    ...
    @contract_tool(app)
    def get_zz500_stocks(date: OptionalISODate = None) -> str:
        """
        获取指定日期的中证500指数成分股数据
