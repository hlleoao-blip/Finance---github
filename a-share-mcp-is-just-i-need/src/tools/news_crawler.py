...
from src.tools.contracts import SearchQuery, ToolExecutionError, TopK, contract_tool
...
def register_news_crawler_tools(app: FastMCP, data_source: FinancialDataSource):
    ...
    @contract_tool(app)
    def crawl_news(query: SearchQuery, top_k: TopK = 10) -> str:
        """
        爬取相关新闻
        ...
        try:
            ...
            raise ToolExecutionError(
                "NEWS_CRAWL_FAILED",
                "Failed to retrieve news for the requested query.",
                retryable=True,
