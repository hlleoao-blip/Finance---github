...
def _format_df_to_markdown_without_tabulate(df: pd.DataFrame) -> str:
    """使用Pandas和字符串拼接生成Markdown表格，避免依赖tabulate"""
    if df.empty:
        return "(No data available to display)"

    columns = [_escape_markdown_cell(column) for column in df.columns]
    if not columns:
        return "(No columns available to display)"
...
def format_df_to_markdown(df: pd.DataFrame, max_rows: int = None) -> str:
    ...
    try:
        ...
        logger.warning(
            f"tabulate is unavailable, using built-in Markdown formatter: {e}")
        markdown_table = _format_df_to_markdown_without_tabulate(df_display)
