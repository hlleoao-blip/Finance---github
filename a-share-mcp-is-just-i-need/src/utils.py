...
# --- Baostock上下文管理器 ---
@contextmanager
def _suppress_baostock_stdout():
    """静音 baostock 内部 print，避免改动 Windows 底层控制台句柄"""
...
@contextmanager
def _baostock_process_lock():
    """跨进程串行化 Baostock 登录/查询/登出，避免并发会话互相踢下线"""
    lock_dir = Path(__file__).resolve().parents[1] / ".runtime"
    lock_dir.mkdir(parents=True, exist_ok=True)
    lock_path = lock_dir / "baostock.lock"
    ...
    with open(lock_path, "a+b") as lock_file:
        ...
        if os.name == "nt":
            import msvcrt
            lock_deadline = time.monotonic() + 120
            while True:
                try:
                    # Windows阻塞锁可能抛EDEADLK，改用非阻塞锁主动等待
                    msvcrt.locking(lock_file.fileno(), msvcrt.LK_NBLCK, 1)
                    break
                except OSError:
                    if time.monotonic() >= lock_deadline:
                        raise TimeoutError("Timed out waiting for Baostock process lock")
                    time.sleep(0.2)
            try:
                yield
            finally:
...
def baostock_login_context():
    ...
    with _baostock_process_lock():
        logger.debug("Attempting Baostock login...")
        with _suppress_baostock_stdout():
            lg = bs.login()
