...
class FinancialDataSource(ABC):
    ...
    def resolve_stock_listing(
        ...
        """Resolve A-share security identity and listing metadata.

        At least one of ``code`` and ``code_name`` must be supplied. Implementations
        return all matches so callers can distinguish a unique security from an
