...
from src.tools.tool_policy import filter_tools_for_agent
...
def _apply_agent_policy(tools, agent_name):
    if agent_name is None:
        return tools
    filtered_tools = filter_tools_for_agent(agent_name, tools)
    logger.info(
        "Applied MCP tool policy for %s: %s",
        agent_name,
...
async def _load_all_mcp_tools():
    ...
    # 多个 Agent 会并行启动，锁住初始化过程，避免重复拉起 uv/MCP 子进程
...
async def get_tools_for_agent(agent_name: str):
    """Return the fail-closed, code-level tool set for one registered agent."""
    if not agent_name:
        raise ValueError("agent_name is required; unrestricted tool access is not allowed.")
    tools = await _load_all_mcp_tools()
    return _apply_agent_policy(tools, agent_name)


async def get_all_tools_for_orchestrator():
    """Explicit privileged access used only by trusted orchestration code."""
    return await _load_all_mcp_tools()


async def get_mcp_tools(agent_name=None):
    """Deprecated compatibility API that now requires an explicit agent identity."""
    if agent_name is None:
        raise ValueError(
            "Unrestricted get_mcp_tools() is disabled. Use get_tools_for_agent() "
            "or get_all_tools_for_orchestrator() explicitly."
        )
    return await get_tools_for_agent(agent_name)
