...
async def fundamental_agent(state: AgentState) -> AgentState:
    ...
    if not user_query:
        ...
        return failed_analysis_update(agent_id=agent_name, analysis_type="fundamental", data=current_data, messages=current_messages, metadata=current_metadata, error=current_data["fundamental_analysis_error"])
    ...
    try:
        ...
        try:
            ...
            logger.info(
                f"{SUCCESS_ICON} FundamentalAgent: Successfully loaded {len(mcp_tools)} policy-scoped tools.")
...
请使用可用工具按集中取数清单登记实际证据，不得自行改变目标证券、年份、季度或日期范围发起新取数。清单中的失败或跳过项直接作为数据缺口披露，不得重试；相同财务或分红数据不得重复调用。"""
            ...
            response = await ainvoke_with_retry(
                ...
                input_data,
                logger=logger,
                operation_name="FundamentalAgent ReAct execution",
            ...
            if not analysis_result.success:
                current_data["fundamental_analysis_error"] = analysis_result.error
                logger.error(
                    "%s FundamentalAgent: Output rejected by deterministic quality checks: %s",
                    ERROR_ICON,
            ...
            new_message = {
                "role": "assistant",
                "content": "基本面分析已完成" if analysis_result.success else "基本面分析未通过质量门",
            }
            ...
            return {
                ...
                "analysis_results": {"fundamental": analysis_result},
            }
